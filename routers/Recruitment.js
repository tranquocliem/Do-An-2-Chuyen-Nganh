...
let storage = multer.diskStorage({
  ...
  //đặt tên cho file lưu
  filename: (req, file, cb) => {
    //<ngày hiện tại>_<tên mặc định>
...
//load danh sách tuyển dụng theo ngày đăng mới nhất đã phê duyệt
...
//pagination cho danh sách tin tuyển dụng của tôi
...
//load danh sách tuyển dụng nổi bật đã phê duyệt
recruitmentRouter.get("/getFavouriteRecruitment", (req, res) => {
  Recruitment.aggregate(
    [
      ...
      { $limit: 6 }, //giới hạn document nếu cần
    ],
    (err, rcm) => {
      if (err) {
        res.status(404).json({
...
//load ds tuyển dụng theo thành phố hoặc ngành đã phê duyệt (Không LoadMore)
...
//load ds tuyển dụng theo thành phố hoặc ngành đã phê duyệt (Có LoadMore)
...
//load ds tuyển dụng theo thành phố hoặc chưa phê duyệt
...
//load chi tiết tin tuyển dụng cho update
recruitmentRouter.post(
...
//search recruitment (sử dụng thêm load more)
recruitmentRouter.post("/searchRecruitment", (req, res) => {
  const { searchTerm } = req.body;
  const skip = req.body.skip ? req.body.skip : 0;
  Recruitment.find(
    // { title: { $regex: searchTerm, $options: "$i" }, status: true },
